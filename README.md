...
**Agent Harness는 LLM을 감싸는 인프라스트럭처 소프트웨어 시스템**으로, 에이전트가 현실 세계에서 신뢰성 있게 작동할 수 있도록 도구 제공, 컨텍스트 관리, 안전 장치, 워크플로우 구조를 제공합니다. 핵심은 에이전트 자체(두뇌)가 아니라 **에이전트가 동작하는 몸체(body)** 라는 점입니다.
